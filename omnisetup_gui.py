...
class OmniSetupGUI:
    def __init__(self, root):
        ...
        self.hw_info = get_hardware_info() if self.system == "Linux" else None
        self.checkboxes = {}
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.log_text = scrolledtext.ScrolledText(main_frame, height=8, state='disabled',
            bg="#000000", fg="#00ff00", insertbackground="#00ff00", font=("Courier", 9))
    ...
    def setup_windows_options(self, parent):
        ...
        ttk.Checkbutton(options_frame, text="Install Visual C++ & .NET Framework",
            variable=self.win_vcpp).pack(anchor=tk.W)

        ttk.Label(options_frame, text="\nWindows Debloat:", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        self.win_debloat = tk.StringVar(value="none")
        ttk.Radiobutton(options_frame, text="None", variable=self.win_debloat, value="none").pack(anchor=tk.W, padx=(20, 0))
        ttk.Radiobutton(options_frame, text="Chris Titus Tech Debloat", variable=self.win_debloat,
            value="christitus").pack(anchor=tk.W, padx=(20, 0))
        ttk.Radiobutton(options_frame, text="Windows10Debloater", variable=self.win_debloat,
            value="windows10debloater").pack(anchor=tk.W, padx=(20, 0))
    ...
    def setup_linux_options(self, parent):
        ...
        ttk.Radiobutton(power_frame, text="None", variable=self.power_base, value="none").pack(anchor=tk.W, padx=(20, 0))
        ttk.Radiobutton(power_frame, text="auto-cpufreq (Automatic CPU optimization)",
            variable=self.power_base, value="auto-cpufreq").pack(anchor=tk.W, padx=(20, 0))
        ttk.Radiobutton(power_frame, text="TLP (Comprehensive power management)",
            variable=self.power_base, value="tlp").pack(anchor=tk.W, padx=(20, 0))
        ...
        if cpu_vendor == 'intel':
            ...
            ttk.Checkbutton(power_frame, text="thermald (Intel thermal management)",
                variable=self.power_tools['thermald']).pack(anchor=tk.W, padx=(20, 0))
        elif cpu_vendor == 'amd':
            self.power_tools['ryzenadj'] = tk.BooleanVar()
            ttk.Checkbutton(power_frame, text="ryzenadj (AMD Ryzen tuning)",
                variable=self.power_tools['ryzenadj']).pack(anchor=tk.W, padx=(20, 0))

        if has_nvidia:
            if cpu_vendor not in ('intel', 'amd'):
                ttk.Label(power_frame, text="\nAdditional Tools:", font=("Arial", 9, "bold")).pack(anchor=tk.W)
            self.power_tools['envycontrol'] = tk.BooleanVar()
            ttk.Checkbutton(power_frame, text="envycontrol (NVIDIA GPU switching)",
                variable=self.power_tools['envycontrol']).pack(anchor=tk.W, padx=(20, 0))
            self.power_tools['optimus-manager'] = tk.BooleanVar()
            ttk.Checkbutton(power_frame, text="optimus-manager (Advanced NVIDIA switching)",
                variable=self.power_tools['optimus-manager']).pack(anchor=tk.W, padx=(20, 0))
    ...
    def setup_apps_section(self, parent):
        ...
        ttk.Label(apps_frame, text="Cross-Platform", font=("Arial", 9, "bold")).grid(
        ...
        for i, app_name in enumerate(self.apps['cross_platform'].keys()):
            var = tk.BooleanVar()
            ttk.Checkbutton(apps_frame, text=app_name, variable=var).grid(
                row=i // 3 + 1, column=i % 3, sticky=tk.W, pady=2)
            self.checkboxes[app_name] = var

        if self.system == "Windows":
            offset = len(self.apps['cross_platform'])
            row_offset = offset // 3 + 2
            ttk.Label(apps_frame, text="Windows Only", font=("Arial", 9, "bold")).grid(
                row=row_offset, column=0, columnspan=3, sticky=tk.W, pady=(10, 2))
            for i, app_name in enumerate(self.apps['windows_only'].keys()):
                var = tk.BooleanVar()
                ttk.Checkbutton(apps_frame, text=app_name, variable=var).grid(
                    row=row_offset + 1 + i // 3, column=i % 3, sticky=tk.W, pady=2)
                self.checkboxes[app_name] = var
    ...
    def install_selected(self):
        if self.system == "Linux":
            ...
            if result.returncode != 0:
                messagebox.showerror("Error", "sudo authentication failed.")
                return
...
def main():
    if platform.system() == "Windows" and not check_git_installed():
        root_temp = tk.Tk()
        root_temp.withdraw()
        response = messagebox.askyesno("Git Not Found",
